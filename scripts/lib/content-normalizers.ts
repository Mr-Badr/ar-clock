...
const EVENT_CONTEXT: Record<string, { descriptor: string; importance: string; practice: string; history: string }> = {
  ...
  'eid-al-adha': {
    ...
    importance: 'يرتبط بالحج والأضاحي وصلاة العيد والإجازات الطويلة، لذلك تحتاج لمعرفة الموعد بدقة وما يرتبط به من أحكام واستعدادات.',
...
const SCHOOL_CONTEXT: Record<string, { authority: string; preparation: string; nuance: string; focus: string }> = {
  ...
  'school-start-morocco': {
    ...
    focus: 'ويظهر في المغرب مصطلح الدخول المدرسي بكثرة، لذلك نربط لك الموعد بالتقويم الرسمي والاستعداد الأسري والمستلزمات في مكان واحد.',
  },
  'school-start-uae': {
...
function buildSchemaDescription(core: EventCoreLike) {
  if (core.slug.includes('results')) {
    return `تابع موعد إعلان نتائج ${core.name}، وتعرف على طريقة الاستعلام الرسمية، واستعد للخطوات التالية بعد ظهور النتيجة عبر مصدر واضح ومحدّث.`;
  }
  if (core.category === 'school') {
