...
function nextFixed(m, d) {
  const now = new Date(); now.setHours(0,0,0,0);
  let t = new Date(now.getFullYear(), m - 1, d); t.setHours(0,0,0,0);
  if (t < now) t = new Date(now.getFullYear() + 1, m - 1, d);
  return t;
}
function nextEstimated(iso) {
  const now = new Date(); now.setHours(0,0,0,0);
  const d = new Date(iso); d.setHours(0,0,0,0);
  if (isNaN(d.getTime()) || d < now) d.setFullYear(d.getFullYear() + 1);
  return d;
}
function nextMonthly(day) {
  const now = new Date(); now.setHours(0,0,0,0);
  let d = new Date(now.getFullYear(), now.getMonth(), day); d.setHours(0,0,0,0);
...
function nextEasterOffset(offset = 0) {
  const now = new Date(); now.setHours(0,0,0,0);
  let e = easterSunday(now.getFullYear());
  let t = new Date(e); t.setDate(t.getDate() + offset); t.setHours(0,0,0,0);
  if (t < now) { e = easterSunday(now.getFullYear() + 1); t = new Date(e); t.setDate(t.getDate() + offset); }
  return t;
}
function roughHijri(hMonth, hDay) {
...
export function getNextEventDate(rawEvent, resolvedMap = {}) {
  ...
  if (ev.type === 'hijri') {
    ...
    return roughHijri(ev.hijriMonth, ev.hijriDay);
  }
  if (ev.type === 'fixed')     return nextFixed(ev.month, ev.day);
  if (ev.type === 'estimated') return nextEstimated(ev.date);
  if (ev.type === 'monthly')   return nextMonthly(ev.day);
  if (ev.type === 'easter')    return nextEasterOffset(ev.easterOffset || 0);
  return new Date();
}
