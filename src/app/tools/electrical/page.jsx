import Link from 'next/link';

import ToolTopAdSlot from '@/components/tools-v2/ToolTopAdSlot';
import { HubGuideSection, HubFaq, buildHubFaqSchema } from '@/components/tools-v2/HubGuideSection';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/components/ui/tooltip';
import { CALCULATOR_ROUTES } from '@/lib/calculators/data';
import { buildCanonicalMetadata } from '@/lib/seo/metadata';
import { getSiteUrl } from '@/lib/site-config';

const SITE_URL = getSiteUrl();

function findRoute(slug) {
  const route = CALCULATOR_ROUTES.find((item) => item.slug === slug);
  if (!route) {
    throw new Error(`electrical hub: no CALCULATOR_ROUTES entry for slug "${slug}"`);
  }
  return route;
}

// Built 2026-07-31 from a real Keyword Planner run (Gulf/Saudi, Arabic) — see
// keyword-research/electrician-hub/DECISION.md. A second pass over the FULL keyword export (not
// just the hand-written candidate list) surfaced real تحويل/حاسبة demand the first pass missed —
// see feedback-analyze-full-keyword-set-not-just-candidates memory — so this hub mixes real
// calculators with editorial guides. Rebuilt 2026-08-01 to match the /tools/construction dot-list
// pattern instead of the old card-grid: every category hub lists tools as dot-lists, calculator
// groups first, editorial group ("المقالات", not "الأدلة") always last — see
// .claude/rules/tools-hub-pattern.md for the standing rule.
const FEATURED_SLUGS = ['electricity-consumption-calculator', 'generators', 'electrical-unit-converter'];

const TYPE_GROUPS = [
  {
    code: 'tools',
    name: 'الأدوات والحاسبات',
    note: 'أرقام مباشرة بدل التخمين — تحويل وحدات وتقدير استهلاك.',
    slugs: ['electrical-unit-converter', 'electricity-consumption-calculator'],
  },
  {
    code: 'articles',
    name: 'المقالات',
    note: 'قبل أي قرار كهربائي — من اختيار المولد إلى قراءة العداد وأرقام الطوارئ.',
    slugs: ['generators', 'breaker-panel', 'meter', 'emergency-numbers'],
  },
];

const FAQ_ITEMS = [
  {
    question: 'كيف أعرف حجم المولد المناسب لمنزلي؟',
    answer: 'اجمع القدرة بالواط لكل الأجهزة التي تريد تشغيلها وقت الانقطاع (مكيف واحد، ثلاجة، إضاءة، إلخ) — لا كل أجهزة المنزل. راجع دليل اختيار المولد لمعرفة معامل القدرة الصحيح ولماذا يحتاج بعض الأجهزة (كالمكيفات) قدرة إضافية عند بدء التشغيل تحديداً.',
  },
  {
    question: 'فاتورة الكهرباء ارتفعت فجأة دون سبب واضح — من أين أبدأ؟',
    answer: 'قبل الشك في عداد معطل، احسب استهلاكك المتوقع في حاسبة استهلاك الكهرباء بإدخال أجهزتك الفعلية وساعات تشغيلها — غالباً يتضح أن السبب جهاز واحد (مكيف يعمل ساعات أطول، سخان مياه قديم) لا خللاً في العداد نفسه.',
  },
  {
    question: 'ما الفرق بين قاطع MCB وقاطع RCBO في لوحة التوزيع؟',
    answer: 'كلاهما يحمي من التحميل الزائد، لكن RCBO يضيف حماية من تسرب التيار الكهربائي (كخطر الصعق عند لمس جهاز به عطل) وهو الأهم في الحمامات والمطابخ تحديداً. راجع دليل لوحة التوزيع لمعرفة متى تحتاج كل نوع.',
  },
];

function ToolLink({ slug }) {
  const route = findRoute(slug);
  return (
    <li>
      <Tooltip>
        <TooltipTrigger asChild>
          <Link href={route.href}>
            <span className="tool-v2-dot" aria-hidden="true">•</span>
            <span className="tool-v2-link-text">{route.shortLabel || route.title}</span>
          </Link>
        </TooltipTrigger>
        <TooltipContent>{route.description}</TooltipContent>
      </Tooltip>
    </li>
  );
}

export const metadata = buildCanonicalMetadata({
  title: 'دليل الكهرباء — المولدات، لوحة التوزيع، العداد، ومحول الوحدات',
  description:
    'اختر حجم مولد الكهرباء المناسب، افهم لوحة التوزيع والقواطع، احسب فاتورتك، واحفظ أرقام طوارئ الكهرباء في الخليج — أدلة وأدوات كهرباء منزلية عملية.',
  keywords: [
    'دليل الكهرباء',
    'مولد كهرباء منزلي',
    'لوحة توزيع الكهرباء',
    'عداد الكهرباء',
    'حاسبة استهلاك الكهرباء',
    'رقم طوارئ الكهرباء',
    'محول وحدات الكهرباء',
    'اسعار مولدات الكهرباء في السعودية',
    'ادوات كهرباء منزلية بالعربي',
  ],
  url: `${SITE_URL}/tools/electrical`,
});

export default function ElectricalHubPage() {
  const allListedSlugs = new Set(TYPE_GROUPS.flatMap((g) => g.slugs));
  const toolCount = allListedSlugs.size;

  const breadcrumbSchema = {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'الرئيسية', item: `${SITE_URL}/` },
      { '@type': 'ListItem', position: 2, name: 'الأدوات', item: `${SITE_URL}/tools` },
      { '@type': 'ListItem', position: 3, name: 'الكهرباء', item: `${SITE_URL}/tools/electrical` },
    ],
  };
  const collectionSchema = {
    '@context': 'https://schema.org',
    '@type': 'CollectionPage',
    name: 'دليل الكهرباء',
    url: `${SITE_URL}/tools/electrical`,
    mainEntity: {
      '@type': 'ItemList',
      numberOfItems: toolCount,
      itemListElement: Array.from(allListedSlugs).map((slug, index) => {
        const route = findRoute(slug);
        return {
          '@type': 'ListItem',
          position: index + 1,
          name: route.shortLabel || route.title,
          url: `${SITE_URL}${route.href}`,
        };
      }),
    },
  };
  const faqSchema = buildHubFaqSchema(FAQ_ITEMS);

  return (
    <main className="bg-base text-primary" dir="rtl" lang="ar">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbSchema) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(collectionSchema) }} />
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }} />

      <ToolTopAdSlot slotId="top-electrical-hub" />

      <div className="container mx-auto px-4 tool-v2-hub-content">
        <div className="tool-v2-cat-hero">
          <div className="tool-v2-cat-hero-top">
            <span className="tool-v2-cat-ic" aria-hidden="true">
              <svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6">
                <path d="M13 2 3 14h7l-1 8 10-12h-7l1-8Z" />
              </svg>
            </span>
            <h1>دليل الكهرباء</h1>
          </div>
          <p>
            من اختيار حجم مولد يكفي فعلاً وقت الانقطاع، إلى فهم لوحة التوزيع والقواطع، إلى حل
            مشاكل العداد والفاتورة، إلى تحويل الوحدات وتقدير استهلاكك — أدلة وأدوات عملية تجيبك
            مباشرة.
          </p>
          <div className="tool-v2-cat-meta">
            <span><b>{toolCount}</b> أدوات وأدلة مرتبطة مباشرة</span>
          </div>
        </div>

        <div className="tool-v2-featured-row">
          {FEATURED_SLUGS.map((slug) => {
            const route = findRoute(slug);
            return (
              <Link key={slug} href={route.href} className="tool-v2-featured-tool">
                <span className="tool-v2-ft-ic" aria-hidden="true">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
                    <path d="M13 2 3 14h7l-1 8 10-12h-7l1-8Z" />
                  </svg>
                </span>
                <span>
                  <b>{route.shortLabel || route.title}</b>
                  <span>{route.badge}</span>
                </span>
              </Link>
            );
          })}
        </div>

        <TooltipProvider>
          <div className="tool-v2-type-groups">
            {TYPE_GROUPS.map((group) => (
              <div key={group.code} className="tool-v2-type-group">
                <h2>{group.name}</h2>
                {group.note ? <p className="tool-v2-type-group-note">{group.note}</p> : null}
                <ul className="tool-v2-tool-link-list">
                  {group.slugs.map((slug) => (
                    <ToolLink key={slug} slug={slug} />
                  ))}
                </ul>
              </div>
            ))}
          </div>
        </TooltipProvider>

        <HubGuideSection id="hub-faq" title="الأسئلة الشائعة">
          <HubFaq items={FAQ_ITEMS} />
        </HubGuideSection>
      </div>
    </main>
  );
}
