    ...
    <div className={`${isEmbedMode ? 'h-full w-full overflow-hidden' : ''}`}>
      ...
      {!isEmbedMode && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* About Section */}
          <div className="lg:col-span-2 bg-surface-3/30 border border-border rounded-2xl p-8">
            ...
            <div className="flex flex-col gap-4">
              <div className="p-4 bg-surface-3 rounded-xl border border-border">
                <span className="text-sm text-muted block mb-1">الموعد المحدد:</span>
              ...
              <div className="p-4 bg-surface-3 rounded-xl border border-border">
                <span className="text-sm text-muted block mb-1">دقة الحساب:</span>
          ...
          <div className="space-y-6">
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  ...
                  <button
                    ...
                    className="bg-surface-3 hover:bg-surface-4 text-primary text-primary px-4 h-10 rounded-sm font-bold text-sm hover:opacity-90 transition-opacity whitespace-nowrap"
                  >
                    Embed
                  </button>
                </div>
                ...
                {typeof navigator !== 'undefined' && navigator.share && (
                  <button
                    ...
                    className="w-full bg-accent-soft text-accent hover:bg-accent-soft-hover py-2.5 rounded-xl font-bold text-sm hover:bg-accent-soft transition-colors"
                  >
            ...
            {showEmbed && (
              <div className="fixed inset-0 bg-black/80 z-[200] flex items-center justify-center p-4 backdrop-blur-md">
                <div className="bg-surface-2 border border-border rounded-2xl p-6 max-w-2xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                  <h3 className="text-xl font-bold mb-4">تخصيص وتضمين العداد</h3>

                  <div className="flex flex-col gap-6 overflow-y-auto">
                    <div>
                      <span className="text-xs font-bold mb-2 block text-muted">اختر مقاس التضمين:</span>
                      <select
                        className="w-full bg-surface-3 border border-border rounded-xl px-4 py-3 text-sm outline-none focus:border-accent transition-colors appearance-none cursor-pointer font-bold"
                        onChange={(e) => {
                    ...
                    <div className="flex-1 flex flex-col min-h-[350px]">
                      <span className="text-xs font-bold mb-2 block text-muted">معاينة مباشرة:</span>
                      <div className="flex-1 bg-black/40 rounded-2xl border border-border flex items-center justify-center p-4 md:p-8 overflow-auto relative">
                        <div
                          className="bg-base shadow-2xl rounded-xl overflow-hidden transition-all duration-300 relative border border-white/5 flex flex-col"
                          style={{ width: `${embedSize.width}px`, height: `${embedSize.height}px` }}
                        >
                          <div className="h-6 bg-surface-3/20 border-b border-border/50 flex items-center px-2 gap-1.5 select-none shrink-0">
                    ...
                    <div>
                      <span className="text-xs font-bold mb-2 block text-muted">كود التضمين النهائي:</span>
                      <div className="bg-black/60 p-4 rounded-xl font-mono text-[10px] break-all border border-border relative group">
                        <code className="text-accent whitespace-pre-wrap">{embedCode}</code>
                        <button
                          onClick={() => copyToClipboard(embedCode, 'تم نسخ كود التضمين!')}
                          className="absolute top-2 left-2 p-2 bg-accent/20 hover:bg-accent-soft rounded-lg text-accent transition-all opacity-0 group-hover:opacity-100 shadow-lg backdrop-blur-md"
                        >
                  ...
                  <div className="flex gap-4 mt-6">
                    <button
                      onClick={() => copyToClipboard(embedCode, 'تم نسخ كود التضمين!')}
                      className="flex-[2] btn btn-primary py-3 rounded-xl font-bold hover:bg-accent-dark transition-all shadow-lg shadow-primary/20 active:scale-[0.98]"
                    >
                      نسخ الكود
                    </button>
                    <button
                      onClick={() => setShowEmbed(false)}
                      className="flex-1 bg-surface-2 border border-border py-3 rounded-xl font-bold hover:bg-surface-3 transition-all active:scale-[0.98]"
                    >
                      إلغاء
                    </button>
                  </div>
            ...
            <Link href="/holidays" className="block bg-surface-3/30 border border-border hover:border-accent rounded-2xl p-6 transition-all group">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-bold mb-1">كل المناسبات</h3>
                  <p className="text-sm text-muted">عرض قائمة المناسبات القادمة</p>
                </div>
                <ArrowRight className="w-6 h-6 transform rotate-180 group-hover:-translate-x-2 transition-transform text-accent" />
              </div>
            </Link>
          </div>
        </div>
