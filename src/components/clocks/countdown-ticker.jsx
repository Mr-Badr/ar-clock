...
import { getTimeRemaining, formatNumber } from '@/lib/holidays-engine';
import FullscreenClock from '@/components/clocks/fullscreen-clock';
import { Share2, Info, ChevronRight, ArrowRight, Monitor, Minimize2 } from 'lucide-react';
import Link from 'next/link';
import { storage } from '@/lib/storage';

export default function CountdownTicker({ holiday, targetDate, initialTimeRemaining, settings: initialSettings, isEmbedInitial }) {
  const [timeRemaining, setTimeRemaining] = useState(initialTimeRemaining);
  const [settings, setSettings] = useState(initialSettings);
  const [isEmbedMode, setIsEmbedMode] = useState(isEmbedInitial);
  const [showEmbed, setShowEmbed] = useState(false);
  const [embedSize, setEmbedSize] = useState({ width: '475', height: '250' });
  ...
  useEffect(() => {
    // Detect embed mode from URL on client
  ...
  const localFormatNumber = (num) => {
    return settings.useArabicNumerals 
      ? num.toString().replace(/\d/g, d => '٠١٢٣٤٥٦٧٨٩'[d])
      : num.toString().padStart(2, '0');
  };

  const formattedDate = new Date(targetDate).toLocaleDateString('ar-SA', { 
      ...
      <div className={`${isEmbedMode ? 'h-full w-full overflow-hidden flex items-center justify-center' : 'relative mb-16'}`}>
              ...
              {timeRemaining && (
                <div className="flex flex-wrap justify-center gap-6 md:gap-16 mb-12">
                  <div className="flex flex-col items-center min-w-[100px]">
                    <span className="text-6xl md:text-9xl font-bold font-mono" style={{ color: settings.clockColor }}>{localFormatNumber(timeRemaining.days)}</span>
                    <span className="text-lg md:text-2xl text-foreground-muted mt-2 uppercase tracking-widest">يوم</span>
                  </div>
                  <div className="flex flex-col items-center min-w-[100px]">
                    <span className="text-6xl md:text-9xl font-bold font-mono" style={{ color: settings.clockColor }}>{localFormatNumber(timeRemaining.hours)}</span>
                    <span className="text-lg md:text-2xl text-foreground-muted mt-2 uppercase tracking-widest">ساعة</span>
                  </div>
                  <div className="flex flex-col items-center min-w-[100px]">
                    <span className="text-6xl md:text-9xl font-bold font-mono" style={{ color: settings.clockColor }}>{localFormatNumber(timeRemaining.minutes)}</span>
                    <span className="text-lg md:text-2xl text-foreground-muted mt-2 uppercase tracking-widest">دقيقة</span>
                  </div>
                  <div className="flex flex-col items-center min-w-[100px]">
                    <span className="text-6xl md:text-9xl font-bold font-mono" style={{ color: settings.clockColor }}>{localFormatNumber(timeRemaining.seconds)}</span>
                    <span className="text-lg md:text-2xl text-foreground-muted mt-2 uppercase tracking-widest">ثانية</span>
                  </div>
                </div>
              )}
        ...
        >
          <div className={`flex flex-col items-center justify-center bg-background border-border text-center relative overflow-hidden transition-all duration-500
            ...
            <div className={`${isEmbedMode ? 'mb-1 md:mb-2' : 'hidden'}`}>
              <h2 className={`${isEmbedMode ? 'text-lg md:text-3xl' : 'text-xl md:text-2xl'} font-bold text-primary truncate max-w-[95vw]`}>
                {holiday.title}
              </h2>
            </div>
            ...
            <div className={`flex flex-wrap justify-center items-center ${isEmbedMode ? 'gap-2 md:gap-6' : 'gap-6 md:gap-16'} relative z-10 w-full flex-1 min-h-0`}>
              <div className={`flex flex-col items-center ${isEmbedMode ? 'min-w-[40px] md:min-w-[100px]' : 'min-w-[70px]'}`}>
                <span className={`${isEmbedMode ? 'text-4xl md:text-8xl' : 'text-5xl md:text-8xl'} font-bold font-mono whitespace-nowrap leading-none transition-all`} style={{ color: settings.clockColor }}>
                  {localFormatNumber(timeRemaining.days)}
                </span>
                <span className={`${isEmbedMode ? 'text-[10px] md:text-base' : 'text-xs md:text-lg'} text-foreground-muted uppercase tracking-tighter md:tracking-widest mt-0.5`}>يوم</span>
              </div>
              
              <div className={`flex flex-col items-center ${isEmbedMode ? 'min-w-[40px] md:min-w-[100px]' : 'min-w-[70px]'}`}>
                <span className={`${isEmbedMode ? 'text-4xl md:text-8xl' : 'text-5xl md:text-8xl'} font-bold font-mono whitespace-nowrap leading-none transition-all`} style={{ color: settings.clockColor }}>
                  {localFormatNumber(timeRemaining.hours)}
                </span>
                <span className={`${isEmbedMode ? 'text-[10px] md:text-base' : 'text-xs md:text-lg'} text-foreground-muted uppercase tracking-tighter md:tracking-widest mt-0.5`}>ساعة</span>
              </div>

              <div className={`flex flex-col items-center ${isEmbedMode ? 'min-w-[40px] md:min-w-[100px]' : 'min-w-[70px]'}`}>
                <span className={`${isEmbedMode ? 'text-4xl md:text-8xl' : 'text-5xl md:text-8xl'} font-bold font-mono whitespace-nowrap leading-none transition-all`} style={{ color: settings.clockColor }}>
                  {localFormatNumber(timeRemaining.minutes)}
                </span>
                <span className={`${isEmbedMode ? 'text-[10px] md:text-base' : 'text-xs md:text-lg'} text-foreground-muted uppercase tracking-tighter md:tracking-widest mt-0.5`}>دقيقة</span>
              </div>

              <div className={`flex flex-col items-center ${isEmbedMode ? 'min-w-[40px] md:min-w-[100px]' : 'min-w-[70px]'}`}>
                <span className={`${isEmbedMode ? 'text-4xl md:text-8xl' : 'text-5xl md:text-8xl'} font-bold font-mono whitespace-nowrap leading-none transition-all`} style={{ color: settings.clockColor }}>
                  {localFormatNumber(timeRemaining.seconds)}
                </span>
                <span className={`${isEmbedMode ? 'text-[10px] md:text-base' : 'text-xs md:text-lg'} text-foreground-muted uppercase tracking-tighter md:tracking-widest mt-0.5`}>ثانية</span>
              </div>
            </div>
              ...
              <div className="p-4 bg-secondary/50 rounded-xl border border-border">
                ...
                <span className="text-lg font-bold">
                  {holiday.type === 'hijri' || !holiday.type ? (
                    `${localFormatNumber(holiday.hijriDay || holiday.day)} ${holiday.name.includes('رمضان') ? 'رمضان' : holiday.name.includes('عيد الفطر') ? 'شوال' : 'في التقويم الهجري'}`
                  ) : (
                    `${localFormatNumber(holiday.day)} / ${localFormatNumber(holiday.month)} (ميلادي)`
                  )}
                </span>
              </div>
              <div className="p-4 bg-secondary/50 rounded-xl border border-border">
              ...
              <div className="relative z-10">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  <a href={shareLinks.facebook} target="_blank" rel="noopener noreferrer" 
                    ...
                    <svg className="w-6 h-6 text-white text-3xl" viewBox="0 0 24 24" fill="currentColor"><path d="M22.675 0h-21.35C.597 0 0 .597 0 1.325v21.351C0 23.403.597 24 1.325 24H12.82v-9.294H9.692v-3.622h3.128V8.413c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.795.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.763v2.313h3.587l-.467 3.622h-3.12V24h6.116c.73 0 1.325-.597 1.325-1.325V1.325C24 .597 23.403 0 22.675 0z"/></svg>
                  </a>
                ...
                {/* Mobile System Share Fallback */}
                {typeof navigator !== 'undefined' && navigator.share && (
                  <button 
                    onClick={() => {
                      navigator.share({
                        title: holiday.title,
                        text: `كم باقي على ${holiday.name}؟ تفقد العداد التنازلي هنا:`,
              ...
              <div className="fixed inset-0 bg-black/80 z-[200] flex items-center justify-center p-4 backdrop-blur-md">
                <div className="bg-surface border border-border rounded-2xl p-6 max-w-2xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                  ...
                  <div className="flex flex-col gap-6 overflow-y-auto">
                    {/* Control Panel: Single Select for Dimensions */}
                    <div>
                      ...
                      <select 
                        ...
                        onChange={(e) => {
                          ...
                          if (size) setEmbedSize({ width: size.width, height: size.height });
                        }}
                        value={WIDGET_SIZES.find(s => s.width === embedSize.width && s.height === embedSize.height)?.label || WIDGET_SIZES[0].label}
                      >
                    ...
                    <div className="flex-1 flex flex-col min-h-[350px]">
                      ...
                      <div className="flex-1 bg-black/40 rounded-2xl border border-border flex items-center justify-center p-4 md:p-8 overflow-auto relative">
                        <div 
                          ...
                          style={{ 
                            width: embedSize.width.includes('%') ? '100%' : `${embedSize.width}px`, 
                      ...
                      <div className="bg-black/60 p-4 rounded-xl font-mono text-[10px] break-all border border-border relative group">
                        ...
                        <button 
                          onClick={() => {
                            ...
                            import('sonner').then(({ toast }) => toast.success('تم نسخ كود التضمين!'));
                          }}
