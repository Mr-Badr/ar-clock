...
export default function TimeNowHero({
  ...
  useEffect(() => {
    const tick = () => setTd(getTimeData(ianaTimezone));
    tick();
        ...
        <div style={{
          ...
          <div style={{ width:'100%', height:'100%', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'1rem', transform:scaleValue, transition:'transform 0.5s ease-in-out', gap:'clamp(1.5rem, 4vh, 3.5rem)' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:'0.5rem' }}>
              <p style={{ margin:0, fontSize:'clamp(1rem, 2.5vw, 1.5rem)', color:'var(--text-secondary)', padding:'0.6rem 1.75rem', background:'color-mix(in srgb, var(--bg-surface-3) 50%, transparent)', border:'1px solid var(--border-default)', borderRadius:'999px' }}>{t.dateAr}</p>
              <p style={{ margin:0, fontSize:'clamp(0.85rem, 2vw, 1.2rem)', color:'var(--text-muted)', padding:'0.4rem 1.25rem', background:'color-mix(in srgb, var(--bg-surface-3) 30%, transparent)', border:'1px solid var(--border-subtle)', borderRadius:'999px' }}>{t.dateHijri}</p>
            </div>

            <span style={{ fontSize:'clamp(0.8rem, 1.5vw, 1.1rem)', color:'var(--text-muted)' }}>
              🌐 {utcOffset} · {ianaTimezone}
            </span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'var(--space-5)', animation:'ct-card-enter 0.8s cubic-bezier(0.175,0.885,0.32,1.275) both' }}>
        <div
          ...
          style={{
            ...
            background:'var(--clock-bg)', backdropFilter:'blur(20px)', WebkitBackdropFilter:'blur(20px)',
            boxShadow:'var(--shadow-accent)',
            padding:'clamp(1.25rem, 3.5vh, 2rem) clamp(1.5rem, 4vw, 3rem)',
            display:'flex', flexDirection:'column', gap:'clamp(1.25rem, 3vh, 2rem)',
          }}
        >
          {/* Toolbar */}
          ...
          <div style={{ display:'flex', justifyContent:'center' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'0.4rem', padding:'0.3rem 0.9rem', borderRadius:'999px', background:'var(--accent-soft)', border:'1px solid var(--border-accent)' }}>
              <span style={{ fontSize:'0.85rem' }} aria-hidden>📍</span>
          ...
          <div
            ...
            style={{ alignItems:'center', justifyContent:'center', gap:'clamp(0.5rem, 3cqi, 2.5rem)', direction:'ltr' }}
          ...
          {/* Date pills */}
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:'0.5rem', animation:'ct-fade-in 0.6s ease 0.55s both' }}>
            {t.dateAr && (
              <span style={{ display:'inline-flex', alignItems:'center', gap:'0.4em', padding:'0.35rem 1.1rem', borderRadius:'999px', background:'var(--bg-surface-3)', border:'1px solid var(--border-subtle)', fontSize:'0.85rem', fontWeight:'500', color:'var(--text-muted)' }}>
                <span aria-hidden style={{ opacity:0.6 }}>📅</span>{t.dateAr}
              </span>
            )}
            {t.dateHijri && (
              <span style={{ display:'inline-flex', alignItems:'center', gap:'0.4em', padding:'0.3rem 0.9rem', borderRadius:'999px', background:'var(--bg-surface-2)', border:'1px solid var(--border-subtle)', fontSize:'0.78rem', fontWeight:'500', color:'var(--text-muted)' }}>
                <span aria-hidden style={{ opacity:0.6 }}>☪️</span>{t.dateHijri}
              </span>
            )}
            <span style={{ display:'inline-flex', alignItems:'center', gap:'0.4em', padding:'0.2rem 0.75rem', borderRadius:'999px', background:'var(--bg-surface-2)', border:'1px solid var(--border-subtle)', fontSize:'0.72rem', fontWeight:'500', color:'var(--text-muted)' }}>
              🌐 {utcOffset} · {ianaTimezone}
            </span>
          </div>
        </div>
