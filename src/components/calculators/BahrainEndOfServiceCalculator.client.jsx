...
import {
  ...
  calculateBahrainEndOfServiceBenefit,
  formatCurrency,
  formatDateArabic,
...
const TERMINATION_OPTIONS = [
  ...
  { value: 'resignation',          label: 'استقالة',                    hint: 'يُطبَّق تخفيض حسب مدة الخدمة (0% / 33% / 67% / 100%).' },
];
...
export default function BahrainEndOfServiceCalculator({ initialStartDate, initialEndDate }) {
  ...
  const fmt = (v) => formatCurrency(v, 'BHD');

  const result = useMemo(
    () => calculateBahrainEndOfServiceBenefit({ basicSalary: salary, startDate, endDate, reason }),
    [salary, startDate, endDate, reason],
  );
  const comparison = useMemo(
    () => buildBahrainEndOfServiceComparison({ salary, startDate, endDate, waitMonths: waitMonths[0], reason }),
    [salary, startDate, endDate, waitMonths, reason],
  );
  const milestones = useMemo(() => buildBahrainEndOfServiceMilestones(startDate), [startDate]);

  const selectedOpt = TERMINATION_OPTIONS.find((o) => o.value === reason);

  const shareText = result.isValid
    ? `مكافأة نهاية الخدمة (البحرين): ${fmt(result.gratuity)}\nمدة الخدمة: ${result.serviceLabel}\nنسبة الاستحقاق: ${result.entitlementPercent}%`
    : '';

  const isZeroResignation = reason === 'resignation' && result.isValid && result.entitlementPercent === 0;

  return (
    <div className="calc-app end-service-tool" aria-label="حاسبة مكافأة نهاية الخدمة البحرين">
            ...
            <CardContent className="calc-esb-form-body">
              ...
              <div className="calc-esb-field">
                <div className="calc-esb-field-label">
                  ...
                  <Label htmlFor="bh-esb-salary">الراتب الأساسي الشهري</Label>
                </div>
              ...
              <div className="calc-esb-field">
                ...
                <div className="calc-esb-date-row">
                  <div className="calc-esb-date-field">
                    <Label htmlFor="bh-esb-start" className="calc-esb-date-label">بداية العمل</Label>
                    <Input id="bh-esb-start" type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} />
                  </div>
                  <div className="calc-esb-date-field">
                    <Label htmlFor="bh-esb-end" className="calc-esb-date-label">نهاية العمل</Label>
                    <Input id="bh-esb-end" type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} />
                  </div>
                </div>
              </div>
        ...
        <div className="calc-esb-result-col">
          {result.isValid ? (
            <div className="calc-esb-result-panel" aria-live="polite">
              ...
              {/* Zero entitlement notice */}
              {isZeroResignation ? (
                <div className="bh-zero-notice">
                  <Warning size={22} weight="fill" />
                  <div>
                    <strong>لا استحقاق عند الاستقالة قبل سنة</strong>
                    <span>وفق قانون العمل البحريني، الاستقالة قبل إتمام سنة كاملة لا تُولِّد أي مكافأة.</span>
                  </div>
                </div>
              ) : (
                <>
                  ...
                  <div className="calc-esb-breakdown">
                    {result.firstThreeYears > 0 && (
                      <div className="calc-esb-brow">
                        <span>
                          {Math.min(result.service.years, 3)} سنة × 15 يوم × {fmt(result.dailyRate)}/يوم
                          <span className="calc-hint"> (أول 3 سنوات)</span>
                        </span>
                        <strong>{fmt(result.firstThreeAmount)}</strong>
                      </div>
                    )}
                    {result.remainingYears > 0 && (
                      <div className="calc-esb-brow">
                        <span>
                          {Math.max(result.service.years - 3, 0)} سنة × 30 يوم × {fmt(result.dailyRate)}/يوم
                          <span className="calc-hint"> (بعد 3 سنوات)</span>
                        </span>
                    ...
                    {reason === 'resignation' && result.entitlementPercent < 100 && (
                      <div className="calc-esb-brow bh-discount-row">
                  ...
                  <ResultActions
                    copyText={shareText}
                    shareTitle="حاسبة مكافأة نهاية الخدمة البحرين"
                    shareText={shareText}
                  />
                        ...
                        <div className="calc-esb-extra-block">
                          ...
                          <div className="calc-esb-timeline">
                            {milestones.map((m) => {
                              ...
                              const msResult = calculateBahrainEndOfServiceBenefit({
                                basicSalary: salary,
                                startDate,
