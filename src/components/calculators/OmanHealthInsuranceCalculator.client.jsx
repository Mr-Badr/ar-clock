...
import {
  ...
  OMAN_HI_TIERS,
  OMAN_HI_AGE_BANDS,
  estimateOmanHealthInsurancePremium,
          ...
          <div key={t.value} className={`calc-hi-class-bar-item${activeTier === t.value ? ' is-active' : ''}`}>
            <div className="calc-hi-class-bar-header">
              ...
              <span className="calc-hi-class-bar-range">
                {fmt(t.perPerson.min)}–{fmt(t.perPerson.max)} ر.ع/سنة
              </span>
            </div>
...
export default function OmanHealthInsuranceCalculator() {
  ...
  const result = useMemo(
    () => estimateOmanHealthInsurancePremium({ tier, familySize, mainAge, preExisting }),
    [tier, familySize, mainAge, preExisting],
  );

  const selectedTier = OMAN_HI_TIERS.find((t) => t.value === tier);
  const ageFactor    = OMAN_HI_AGE_BANDS.find((a) => a.value === mainAge)?.factor ?? 1;
  ...
  const shareText = result.isValid
    ? `التأمين الصحي (عُمان) — ${selectedTier?.label}:\n${memberCount} ${memberCount === 1 ? 'شخص' : 'أشخاص'}\nسنوياً: ${fmt(result.totalMin)}–${fmt(result.totalMax)} ر.ع`
    : '';

  return (
    <div className="calc-app om-health-ins-tool" aria-label="حاسبة التأمين الصحي عُمان">
            ...
            <div className="calc-esb-result-panel" aria-live="polite">
              ...
              <div className="calc-esb-amount-hero">
                ...
                <div className="ci-range-display">
                  ...
                  <span className="ci-range-unit">ر.ع</span>
                </div>
              ...
              <div className="calc-insight-row">
                ...
                <div className="calc-insight-card">
                  ...
                  <span className="calc-insight-sub">ر.ع/سنة</span>
                </div>
              </div>
              ...
              <div className="calc-esb-breakdown">
                ...
                <div className="calc-esb-brow calc-esb-brow--total">
                  ...
                  <strong>{fmt(result.totalMin)}–{fmt(result.totalMax)} ر.ع</strong>
                </div>
              </div>
              ...
              <ResultActions copyText={shareText} shareTitle="حاسبة التأمين الصحي عُمان" shareText={shareText} />

              <div className="calc-esb-reason-strip">
                <Warning size={14} weight="fill" />
                <span>هذه أرقام تأمين تكميلي (فوق التأمين الإلزامي بموجب القرار الوزاري 76/2019) استرشادية من السوق العُماني — الأسعار الفعلية تختلف بين الشركات. احصل على عرض رسمي لتأكيد السعر.</span>
              </div>
            </div>
          ) : (
