...
import {
  QATAR_AREA_FACTORS,
  QATAR_DRIVER_AGE_BANDS,
  NO_CLAIM_DISCOUNTS,
  estimateQatarCarInsurancePremium,
  getQatarPriceFactors,
    ...
    <div className="calc-donut-wrap">
      <div className="calc-donut-center-wrap">
        ...
        <div className="calc-donut-center-text">
          <span className="calc-donut-center-pct" style={{ fontSize: '0.82rem' }}>{fmt(low)}–</span>
          <span className="calc-donut-center-pct">{fmt(high)}</span>
          <span className="calc-donut-center-sub">ر.ق / سنة</span>
        </div>
      </div>
...
export default function QatarCarInsuranceCalculator() {
  ...
  const result = useMemo(
    () => estimateQatarCarInsurancePremium({ coverageType: coverage, carValue, carAge, driverAge, area, noClaimYears }),
    [coverage, carValue, carAge, driverAge, area, noClaimYears],
  );
  const factors = useMemo(
    () => getQatarPriceFactors({ coverageType: coverage, carValue, carAge, driverAge, area, noClaimYears }),
    [coverage, carValue, carAge, driverAge, area, noClaimYears],
  );
  ...
  const shareText = result.isValid
    ? `تأمين السيارة (قطر):\nنوع: ${coverage === 'third-party' ? 'ضد الغير' : 'شامل'}\nالتقدير: ${fmt(result.low)} – ${fmt(result.high)} ر.ق/سنة`
    : '';

  return (
    <div className="calc-app qa-car-ins-tool" aria-label="حاسبة تأمين السيارة قطر">
            ...
            <CardContent className="calc-esb-form-body">
              ...
              <div className="calc-esb-field">
                ...
                <p className="calc-hint">
                  <Info size={11} /> التأمين ضد الغير إلزامي لجميع المركبات في قطر.
                </p>
              </div>
              ...
              {coverage === 'comprehensive' && (
                <div className="calc-esb-field">
                  <div className="calc-esb-field-label">
                    ...
                    <Label htmlFor="qa-val">قيمة السيارة السوقية</Label>
                  </div>
                  <div className="calc-esb-money-row">
                    <Input id="qa-val" inputMode="numeric" value={carValue}
                      onChange={(e) => setCarValue(e.target.value)} placeholder="60000" />
              ...
              <div className="calc-esb-field">
                ...
                <Select value={area} onValueChange={setArea}>
                  <SelectTrigger className="w-full"><SelectValue /></SelectTrigger>
                  <SelectContent>
                    {Object.entries(QATAR_AREA_FACTORS).map(([k, v]) => (
                  ...
                  <SelectContent>
                    {Object.entries(NO_CLAIM_DISCOUNTS).map(([k, v]) => (
                      <SelectItem key={k} value={k}>
                        {v.label} {v.discount > 0 ? `(خصم ${v.discount * 100}%)` : ''}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
            ...
            <div className="calc-esb-result-panel" aria-live="polite">
              ...
              <div className="calc-esb-amount-hero">
                ...
                <div className="calc-esb-amount-meta">
                  <span>متوسط ~{fmt(monthly)} ر.ق/شهر</span>
                </div>
              </div>
              ...
              <div className="calc-insight-row">
                <div className="calc-insight-card">
                  ...
                  <span className="calc-insight-value">{fmt(monthly)} ر.ق</span>
                  ...
                  <div className="ci-factors">
                    {factors.map((f, i) => (
                      <div key={i} className="ci-factor-row">
                        <span className={`ci-factor-icon ci-factor-icon--${f.icon}`} aria-hidden="true">
                          {FACTOR_ICONS[f.icon]}
                        </span>
              ...
              <ResultActions copyText={shareText} shareTitle="حاسبة تأمين السيارة قطر" shareText={shareText} />

              <div className="calc-esb-reason-strip">
                <Warning size={14} weight="fill" />
                <span>تقدير استرشادي — احصل على عرض رسمي من شركة تأمين مرخصة من قِبل مصرف قطر المركزي.</span>
              </div>
            </div>
          ) : (
